...
│ │ │ │   ├── houses/                                                         # Модели для тестов house
...
│   │ │ │ ├── base/                                                           # Пакет с базовыми тестами для API
│   │ │ │ │ └── BaseApiTest.java                                              # Базовый тест для всех API тестов
│   │ │ │ ├── cars/                                                           # Пакет с API тестами по авто
│   │ │ │ │ └── CarsApiTest.java                                              # API-тесты для Cars
│   │ │ │ ├── houses/                                                         # Пакет с API тестами по домам
│   │ │ │ │ └── HousesApiTest.java                                            # API-тесты для House
...
| №        | Тест-кейс                                                          | Группа                           | Автор        | Статус реализации |
|----------|--------------------------------------------------------------------|----------------------------------|--------------|-------------------|
| АТ.05.01 | Проверка открытия страницы "Users -> Read all" со всеми атрибутами | Positive, E2E, Regression, Smoke | Светлов А.А. | Готов             |
| АТ.05.02 | Проверка ховер эффекта кнопок                                      | Positive, Regression             | Светлов А.А. | Готов             |
| АТ.05.03 | Проверка сортировки пользователей по возрастанию ID                | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.04 | Проверка сортировки пользователей по убыванию ID                   | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.05 | Проверка сортировки пользователей по имени от A-Z                  | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.06 | Проверка сортировки пользователей по имени от Z-A                  | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.07 | Проверка сортировки пользователей по фамилии от A-Z                | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.08 | Проверка сортировки пользователей по фамилии от Z-A                | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.09 | Проверка сортировки пользователей по возрастанию возраста          | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.10 | Проверка сортировки пользователей по убыванию возраста             | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.11 | Проверка сортировки пользователей по полу от A-Z                   | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.12 | Проверка сортировки пользователей по полу от Z-A                   | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.13 | Проверка сортировки пользователей по возрастанию наличности        | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.14 | Проверка сортировки пользователей по убыванию наличности           | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.05.15 | Проверка обновления таблицы                                        | Positive, E2E, Regression        | Светлов А.А. | Готов             |

#### Таблица пользователей и их машин (Users -> Read user with cars)

| №        | Тест-кейс                                                                     | Группа                           | Автор        | Статус реализации |
|----------|-------------------------------------------------------------------------------|----------------------------------|--------------|-------------------|
| АТ.06.01 | Проверка открытия страницы "Users -> Read user with cars" со всеми атрибутами | Positive, E2E, Regression, Smoke | Светлов А.А. | Готов             |
| АТ.06.02 | Проверка ввода в инпут айди напрямую и через стрелочки                        | Positive, Regression, Smoke      | Светлов А.А. | Готов             |
| АТ.06.03 | Проверка ховер-эффектов и тултипов                                            | Positive, Regression             | Светлов А.А. | Готов             |
| АТ.06.04 | Проверка отображения данных по пользователю без машины                        | Positive, E2E, Regression        | Светлов А.А. | Готов             |
| АТ.06.05 | Проверка отображения данных по пользователю c одной машиной                   | Positive, E2E, Regression        | Светлов А.А. | Готов             |

#### Вызов данных по пользователям с машинами (Users -> Read user with cars) — негативные тесты

| №        | Тест-кейс                                                                                                   | Группа               | Автор        | Статус реализации |
|----------|-------------------------------------------------------------------------------------------------------------|----------------------|--------------|-------------------|
| АТ.06.06 | Проверка валидации полей с получением ошибок при вводе невалидных значений (DataProvider: 4 наборов данных) | Negative, Regression | Светлов А.А. | Готов             |

#### Детализация DataProvider АТ.06.07:

| № | Невалидные данные                                                | Описание                              | Автор        | Статус реализации |
|---|------------------------------------------------------------------|---------------------------------------|--------------|-------------------|
| 1 | userId = "999999", ожидаемый статус "Status: 204 user not found" | Запрос данных по несуществующему айди | Светлов А.А. | Готов             |
| 2 | userId = "-999999", ожидаемый статус Status:  Invalid input"     | Запрос данных по отрицательному айди  | Светлов А.А. | Готов             |
| 3 | userId = ",", ожидаемый статус "Status: 204 user not found"      | Запрос данных по невалидному айди     | Светлов А.А. | Готов             |
| 4 | userId = "", ожидаемый статус "Status: 204 user not found"       | Запрос данных по пустому айди         | Светлов А.А. | Готов             |
...
| № | Тест-кейс | Метод | Эндпоинт | Группа          | Автор         | Статус реализации |
|---|-----------|-------|----------|-----------------|---------------|-------------------|
| API.04.01 | Создание дома с валидными данными | POST | `/house` | Positive, Regression, Smoke | Малеваный А.В | Готов     |
| API.04.02 | Создание дома с невалидным токеном | POST | `/house` | Negative, Regression | Малеваный А.В | Готов             |
...
|-------------|---------------|----------------------------------------------------------------------|
| QA Engineer | Антон М.      | Login/Auth · Houses (UI + API)                                       |
| QA Engineer | Олег П.       | All POST (UI) · Users (API) · All POST/DELETE                        |
| QA Engineer | Егор П.       | Cars (UI + API)                                                      |
| QA Engineer | Александра З. | Users — создание пользователя, пополнение баланса (UI), проверка БД. |
| QA Engineer | Анатолий С.   | Users — Read All, Read With Cars (UI)                                |
